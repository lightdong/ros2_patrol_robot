...
def generate_launch_description():
    ...
    default_xacro_path =  os.path.join(urdf_package_path, 'urdf', 'dongbot/dongbot.urdf.xacro')
    # default_rsiv_path = os.path.join(urdf_package_path, 'config', 'dongbot_config.rviz')
    default_gazebo_world = os.path.join(urdf_package_path, 'world', 'nav_room.world')
    ...
    # 启动gazebo 并且启动默认的世界路径
    action_launch_gazebo = launch.actions.IncludeLaunchDescription(
        launch.launch_description_sources.PythonLaunchDescriptionSource(
            [get_package_share_directory('gazebo_ros'), '/launch','/gazebo.launch.py']
        ),
        launch_arguments=[('world', default_gazebo_world),('verbose','true')]
    )
    ...
    action_load_joint_state_controller = launch.actions.ExecuteProcess(
        cmd='ros2 control load_controller dongbot_joint_state_broadcaster --set-state active'.split(' '),
        output='screen'
    )   

    # 添加动作 激活力控制器
    action_load_effort_controller = launch.actions.ExecuteProcess(
        cmd='ros2 control load_controller dongbot_effort_controller --set-state active'.split(' '),
        output='screen'
    )   

    # 使用差速控制器作为控制器 不再使用
    action_load_diff_driver_controller = launch.actions.ExecuteProcess(
        cmd='ros2 control load_controller dongbot_diff_drive_controller --set-state active'.split(' '),
        output='screen'
    )    
    ...
    return launch.LaunchDescription([
        ...
        launch.actions.RegisterEventHandler(
            event_handler=launch.event_handlers.OnProcessExit(
                ...
                on_exit=[
                    action_load_diff_driver_controller
                ]
            )
        )
    ])